...
export default function Dashboard({ username, onSelectSubmission, onLogout }: DashboardProps) {
  ...
  const filteredSubmissions = submissions.filter((sub) => {
    ...
    if (showAll) {
      ...
      return matchesSearch && matchesType && !sub.isSaved;
    }
  });
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-800" dir="rtl">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-slate-900 p-6 rounded-2xl shadow-md border border-slate-800 text-white flex flex-col justify-between sm:col-span-2 lg:col-span-1">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">تحديث حي ومباشر</span>
            ...
            <p className="text-xs text-slate-400 leading-relaxed mt-2">
              متصل تلقائياً بجدول البيانات لضمان عدم تكرار تصحيح الدروس بين المعلمين.
            </p>
            <button
              onClick={fetchData}
              disabled={loading}
              className="mt-4 flex items-center justify-center gap-2 py-2 px-4 rounded-xl border border-slate-700 hover:border-emerald-500 hover:bg-slate-800 text-xs font-medium text-slate-300 hover:text-emerald-400 transition-all focus:outline-none"
            >
          ...
          <div className="p-6 border-b border-slate-100 flex flex-col xl:flex-row justify-between items-stretch xl:items-center gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3 justify-start xl:justify-end">
              ...
              <button
                ...
                className="flex items-center justify-center gap-1.5 px-4 py-3 bg-slate-100 hover:bg-slate-200 border border-slate-200/80 rounded-xl text-xs font-bold text-slate-700 hover:text-slate-900 transition-all select-none w-full sm:w-auto"
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 px-4 py-3 border rounded-xl text-xs font-semibold transition-all focus:outline-none w-full sm:w-auto ${
                  showAll
              ...
              >
                ...
                {showAll ? 'إظهار المعلق فقط' : 'إظهار الكل (مع المصحح)'}
              </button>
              ...
              <button
                ...
                disabled={loading}
                title="إعادة تحميل"
                className="p-3 border border-slate-200 hover:border-slate-300 rounded-xl hover:bg-slate-50 text-slate-600 transition-all focus:outline-none self-stretch sm:self-auto flex items-center justify-center"
              >
